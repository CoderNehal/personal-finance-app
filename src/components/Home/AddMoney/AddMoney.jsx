...
const AddMoney = () => {
	...
	const UpdateData = async () => {
		...
		axios
			...
			.then(() => {
				setTimeout(() => {
					setShowAlert(true);
					setCurrentBalance(updatedAmmount);
					setAmoutToAdd(0);
				}, 300);
			})
			.catch((err) => {
	...
	const HandleAddMoney = async () => {
		setShowAlert(false);
		if (AmoutToAdd === 0 || AmoutToAdd === null) {
			alert('Please Enter a Valid Amount');
		} else if (ModeOFIncome === null) {
			alert('Please select Mode of Income');
		} else {
			setUpdatedBalance(CurrentBalance + AmoutToAdd);
					...
					<AnimatePresence>
						{showConfirmationModal && (
							<ConfirmationModal
								...
								confirmTransaction={() => UpdateData()}
							/>
						)}
					</AnimatePresence>
					<div className='AddMoneyContainer   relative'>
						<div className='flex flex-col md:flex-row justify-between h-full items-center  '>
							{/*Back Button */}
							<div className='bckbtn md:pl-8 h-full w-full md:w-auto '>
								<button
