...
const SpendMoney = () => {
	...
	const UpdateData = (money, reason, OtherReason) => {
		axios
			...
			.then(() => {
				setTimeout(() => {
					setCurrentBalance(CurrentBalance - AmoutToSpend);
					setShowAlert(true);
					setAmoutToSpend(0);
				}, 300);
			})
	...
	const HandleSpendMoney = async () => {
		...
		if (!other) {
			if (AmoutToSpend == null || AmoutToSpend <= 0) {
			...
			if (!AmoutToSpend <= 0 || AmoutToSpend) {
				setshowConfirmationModal(true);
		...
		<>
			{isLoading ? (
				...
				<>
					...
					<div className='SpendMoneyContainer relative'>
						<div className='flex flex-col md:flex-row justify-between h-full items-center  '>
							{/*Back Button */}
							<div className='bckbtn h-full w-full md:w-auto pl-8 md:p-0'>
								<button
									...
									<div
										...
										{other ? null : (
											<>
												...
												<motion.div
													ref={ref}
													className=' lg:px-8 py-6 overflow-y-hidden flex h-full md:h-1/2 w-3/4 justify-between items-center'>
													<motion.div
													...
													<motion.div
														...
														<img src={Car} alt='' />
														<p className='text-xs md:text-sm text-red font-semibold uppercase  pt-0  md:pt-2  '>
															Transport
														</p>
													</motion.div>
													<motion.div
